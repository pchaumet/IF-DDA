c$$$c****************************************************************
c$$$c     Iterative solver GPBICGAR2
c$$$c****************************************************************
...
c$$$
c$$$      
c$$$ 10   call GPBICGAR2(Xi,XR,B,lda,ndim,nlar,nou,WRK,NLOOP,MAXIT ,TOL
...
c$$$      end
c*************************************************
      SUBROUTINE GPBICGAR2(Xi,XR,B,lda,ndim,nlar,nou,WRK,ITNO,MAXIT,TOL
...
c     initialize r0=b-Ax0,rOt=rO,p0=v0=d0, compute initial residu,  Ar0
 10   R0RN=0.d0
!$OMP PARALLEL DEFAULT(SHARED) PRIVATE(ii) 
...
c     compute p=r+beta*(p-u), Ap=Ar+beta*(Ap-Au);alpha=r0r/r0Ap
      ctmp=0.d0
!$OMP PARALLEL DEFAULT(SHARED) PRIVATE(ii) 
...
!$OMP DO SCHEDULE(STATIC)              
         do ii=1,ndim
            ...
            WRK(ii,5)=DZETA*WRK(ii,4)
            WRK(ii,9)=DZETA*WRK(ii,7)-ALPHA*WRK(ii,12)
         enddo
!$OMP ENDDO 
!$OMP END PARALLEL          
      else
