c     routine qui appelle la FFT de singleton
      subroutine fftsingletonz2d(vect,NX,NY,ISIGN)
      implicit none
      INTEGER i,NX,NY,NZ,ISIGN,ntotal
      double complex vect(*)
      ntotal=nx*ny

      if (isign.eq.1) then
         call fftz(vect,ntotal,NX,NX,1)
         call fftz(vect,ntotal,NY,ntotal,1)
...
c*************************************************************
c     routine qui appelle la FFT de singleton
      subroutine fftsingletonz3d(vect,NX,NY,NZ,ISIGN)
      implicit none
      INTEGER i,NX,NY,NZ,ISIGN,ntotal
      double precision dntotal
      double complex vect(*)
      ntotal=nx*ny*nz
c      dntotal=dble(ntotal)

      if (isign.eq.1) then
         call fftz(vect,ntotal,NX,NX,ISIGN)
         call fftz(vect,ntotal,NY,NX*NY,ISIGN)
         call fftz(vect,ntotal,NZ,ntotal,ISIGN)
      elseif (isign.eq.-1) then
         call fftz(vect,ntotal,NZ,ntotal,ISIGN)
         call fftz(vect,ntotal,NY,NX*NY,ISIGN)
         call fftz(vect,ntotal,NX,NX,ISIGN)
        
c         call fftz(vect,ntotal,NX,NX,-1)
c         call fftz(vect,ntotal,NY,NX*NY,-1)
c         call fftz(vect,ntotal,NZ,ntotal,-1)
      endif
...
c*************************************************************
c*************************************************************
      subroutine fftz(zvect,ntot,n,nspan,isn)
      IMPLICIT REAL*8 (A-H,O-Y)
      double complex zvect(*),zicomp
...
c  transform for factor of 2 (including rotation factor)
  ...
  210 k2=kk+kspan
      ak=dreal(zvect(k2))
      bk=dimag(zvect(k2))
      zvect(k2)=zvect(kk)-ak-bk*zicomp
      zvect(kk)=zvect(kk)+ak+bk*zicomp
c      write(*,*) 'ce',zvect(k2),zvect(kk)
      kk=k2+kspan
  ...
  220 c1=1.0-cd
      s1=sd
  230 k2=kk+kspan
      ak=dreal(zvect(kk)-zvect(k2))
      bk=dimag(zvect(kk)-zvect(k2))
      zvect(kk)=zvect(kk)+zvect(k2)
      zvect(k2)=c1*ak-s1*bk+zicomp*(s1*ak+c1*bk)
      kk=k2+kspan
...
c  transform for factor of 3 (optional code)
  320 k1=kk+kspan
      ...
      aj=dreal(zvect(k1)+zvect(k2))
      bj=dimag(zvect(k1)+zvect(k2))
      zvect(kk)=ak+aj+zicomp*(bk+bj)
c      write(*,*) 'pp',ak,bk,aj,bj
      ak=-0.5d0*aj+ak
      bk=-0.5d0*bj+bk
      
      aj=dreal(zvect(k1)-zvect(k2))*s120
      bj=dimag(zvect(k1)-zvect(k2))*s120

c      write(*,*) 'rr',ak,bk,aj,bj
      zvect(k1)=ak-bj+zicomp*(bk+aj)
      zvect(k2)=ak+bj+zicomp*(bk-aj)
c      write(*,*) 'ce3',zvect(k2),zvect(k1),s120
      kk=k2+kspan
...
c  transform for factor of 4
  ...
  420 k1=kk+kspan
      k2=k1+kspan
      k3=k2+kspan
      akp=dreal(zvect(kk)+zvect(k2))
      akm=dreal(zvect(kk)-zvect(k2))
      ajp=dreal(zvect(k1)+zvect(k3))
      ajm=dreal(zvect(k1)-zvect(k3))

      bkp=dimag(zvect(kk)+zvect(k2))
      bkm=dimag(zvect(kk)-zvect(k2))
      bjp=dimag(zvect(k1)+zvect(k3))
      bjm=dimag(zvect(k1)-zvect(k3))
      zvect(kk)=akp+ajp+zicomp*(bkp+bjp)
c      write(*,*) 'ce4',zvect(kk)
      ajp=akp-ajp
      bjp=bkp-bjp
 ...
 430  zvect(k1)=akp*c1-bkp*s1+zicomp*(akp*s1+bkp*c1)
      zvect(k2)=ajp*c2-bjp*s2+zicomp*(ajp*s2+bjp*c2)
      zvect(k3)=akm*c3-bkm*s3+zicomp*(akm*s3+bkm*c3)
c      write(*,*) 'pppp',zvect(k1),zvect(k2),zvect(k3)
      kk=k3+kspan
 ...
 460  zvect(k1)=akp+zicomp*bkp
      zvect(k2)=ajp+zicomp*bjp
      zvect(k3)=akm+zicomp*bkm
c      write(*,*) 'fpppp',zvect(k1),zvect(k2),zvect(k3)
      kk=k3+kspan
      if(kk .le. nt) go to 420
      go to 440
c  transform for factor of 5 (optional code)
  510 c2=c72**2.d0-s72**2.d0
      s2=2.d0*c72*s72
  520 k1=kk+kspan
      k2=k1+kspan
      k3=k2+kspan
      k4=k3+kspan
      akp=dreal(zvect(k1)+zvect(k4))
      akm=dreal(zvect(k1)-zvect(k4))
      bkp=dimag(zvect(k1)+zvect(k4))
      bkm=dimag(zvect(k1)-zvect(k4))
      ajp=dreal(zvect(k2)+zvect(k3))
      ajm=dreal(zvect(k2)-zvect(k3))
      bjp=dimag(zvect(k2)+zvect(k3))
      bjm=dimag(zvect(k2)-zvect(k3))
      aa=dreal(zvect(kk))
      bb=dimag(zvect(kk))
      zvect(kk)=aa+akp+ajp+zicomp*(bb+bkp+bjp)
c      write(*,*) 'tttt',zvect(kk)
      ak=akp*c72+ajp*c2+aa
      bk=bkp*c72+bjp*c2+bb
      aj=akm*s72+ajm*s2
      bj=bkm*s72+bjm*s2
      zvect(k1)=ak-bj+zicomp*(bk+aj)
      zvect(k4)=ak+bj+zicomp*(bk-aj)
      ak=akp*c2+ajp*c72+aa
      bk=bkp*c2+bjp*c72+bb
      aj=akm*s2-ajm*s72
      bj=bkm*s2-bjm*s72
      zvect(k2)=ak-bj+zicomp*(bk+aj)
      zvect(k3)=ak+bj+zicomp*(bk-aj)
      kk=k4+kspan
...
c  transform for odd factors
  ...
  650 k2=k2-kspan
      j=j+1
      at(j)=dreal(zvect(k1)+zvect(k2))
      ak=at(j)+ak
      bt(j)=dimag(zvect(k1)+zvect(k2))
      bk=bt(j)+bk
      j=j+1
      at(j)=dreal(zvect(k1)-zvect(k2))
      bt(j)=dimag(zvect(k1)-zvect(k2))
...
c  multiply by rotation factor (except for factors of 2 and 4)
  ...
  720 c1=c2
      s2=s1
      kk=kk+kspan
 730  ak=dreal(zvect(kk))
      tmp=dimag(zvect(kk))
      zvect(kk)=c2*ak-s2*tmp+zicomp*(s2*ak+c2*tmp)
      kk=kk+kspnn
...
c  permutation for single-variate transform (optional code)
 820  ak=dreal(zvect(kk))
      bk=dimag(zvect(kk))
      zvect(kk)=zvect(k2)
      zvect(k2)=ak+zicomp*bk
      kk=kk+inc
      k2=kspan+k2
...
c  reorder a and b, following the permutation cycles
  ...
  926 kspan=jj
      ...
      k2=0
  928 k2=k2+1
      at(k2)=dreal(zvect(k1))
      bt(k2)=dimag(zvect(k1))
      k1=k1-inc
      if(k1 .ne. kk) go to 928
  932 k1=kk+kspan
      k2=k1-jc*(k+np(k))
      k=-np(k)
 936  zvect(k1)=zvect(k2)
      k1=k1-inc
      k2=k2-inc
